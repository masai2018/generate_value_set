################################################################################
### import pharmacy data and sort by month
### version controlled by git
################################################################################
## running whole script need around 3 hours on server at 10/10/2018
...
SortByMonth <- function(bigfile, dir.raw = inDir, 
  ...
  while(readnlines == chunksize){
    ...
    mycat(nn)
    mycat('\n # lines = ', nolines, '\n')
    datachunk[, month := format(as.Date(as.character(prescription_filled_dt), format = "%m/%d/%Y"), "%Y_%m")]
    datachunk[, 
              fwrite(.SD, 
                          file = file.loc <- paste0(dir.out, '/', "PHARMACY_",month, ".csv"), 
                          append = TRUE, 
                          sep = ",", 
                          col.names = FALSE,
                          row.names = FALSE),
  ...
  close(con) 
  mycat('END of file: ', bigfile, 'Time: ', as.character(Sys.time()), '\n #', nn, 'chunk \n # lines = ', nolines, '\n')
}
## end of function

## start to sort
for (year in 2017:2012){
  bigfile <- paste0('PHARMACY_', year, '.txt')
  dir.raw <- inDir
  dir.out <- outDir
  sep = '|'
  ...
  for (month in sprintf("%02d", 1:12))
    fwrite(as.data.table(matrix(fhead, nrow = 1)), paste0(dir.out, "PHARMACY_",year, '_', month, ".csv"), 
                sep = ",", row.names = FALSE, col.names = FALSE, eol = "\n")
  SortByMonth(bigfile = paste0('PHARMACY_', year, '.txt'),
              dir.out = dir.out,
              chunksize = 4e5,
              sep = sep, 
              header = T)
